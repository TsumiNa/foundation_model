...
class CompoundPropertyPredictorLightning(pl.LightningModule):
    """
    LightningModule version of CompoundPropertyPredictor supporting duoGPU parallel training.
    """

    def __init__(
        self, input_dim, output_dim, norm_shared=True, norm_tasks=True, lr=0.0005
    ):
        super().__init__()
        self.save_hyperparameters()
        self.norm_shared = norm_shared
        self.norm_tasks = norm_tasks
        self.lr = lr
        self.fc1 = LinearLayer(input_dim, 128, normalization=norm_shared)
        self.fc2 = LinearLayer(128, 64, normalization=norm_shared)
        self.fc3 = LinearLayer(64, 32, normalization=norm_shared, activation=nn.Tanh())
        self.output_layers = nn.ModuleList(
            [
                nn.Sequential(
                    LinearLayer(32, 16, normalization=norm_tasks),
                    LinearLayer(16, 1, normalization=False, activation=None),
                )
    ...
    for fraction in fractions:
        ...
        for run in range(num_runs):
            ...
            model = CompoundPropertyPredictor(
                ...
                output_dim=len(CompoundDataset.ATTRIBUTES),
            ).to(device)
