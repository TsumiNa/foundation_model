...
txt(s, 0.6, 1.3, 12.2, 5.8, [
    ...
    "",
    "4. The epoch budget saturates near ~100: max_epochs=150 adds +0.009 on average (n1000/n2000 flat, n1500/n2500 +0.02) —",
    "    the mechanism is coverage accumulation, not raw extra optimization.",
    "",
...
rows = [
    ...
    ("100", "0.371", "0.475", "0.592", "—"),
    ("200", "0.420", "0.546", "0.606", "—"),
    ("500", "0.498", "0.582", "0.637", "—"),
...
txt(s, 7.8, 1.5, 5.0, 5.2, [
    ...
    "· m150 − p24: −0.001 / +0.019 / −0.005 / +0.024 —",
    "  mean +0.009, within single-seed noise; the epoch",
    "  budget is saturated at ~100 for the resampling effect",
    "· best overall: n2500-m150 (0.663) — at large n the",
    "  replay ≈ full data, so longer training keeps helping",
    "  (a plain training-time effect, not resampling)",
    "",
    "Open control: step-p24 (frozen + long training) would",
    "cleanly separate the two mechanisms — not yet run.",
], size=13)
