...
def fit_gap(xs: np.ndarray, gaps: np.ndarray):
    """gap(x) = g0 / (1 + x/k) via linear regression on 1/gap; None if under-determined."""
    good = gaps > 1e-3
    if good.sum() < 4 or len(set(np.round(xs[good], 6))) < 3:
        return None
    A = np.vstack([np.ones(good.sum()), xs[good]]).T
    coef, *_ = np.linalg.lstsq(A, 1.0 / gaps[good], rcond=None)
    a, b = coef
    if a <= 0 or b <= 0:  # non-decaying gap — the model doesn't describe this task
        return None
...
for ax, task in zip(axes.flat, ORDER):
    ...
    fit = fit_gap(xs_arr, ceiling - ys_arr)
    if fit is not None:
        g0, k = fit
        n90 = 9 * k
        ...
        fit_txt = f"knee≈{k:,.0f}\n90%≈{n90:,.0f}"
        print(f"{task:24} {n_train:>7} {ceiling:>7.3f} {min(ys):>7.3f}..{max(ys):<7.3f} {k:>8,.0f} {n90:>8,.0f} {n90 / n_train:>10.1f}x")
    else:
        fit_txt = "fit n/a"
        print(f"{task:24} {n_train:>7} {ceiling:>7.3f} {min(ys):>7.3f}..{max(ys):<7.3f} {'-':>8} {'-':>8} {'-':>11}")
    ...
    ax.set_xscale("log")
    ax.set_title(f"{task}  (n_train={n_train:,})", fontsize=9)
    ax.text(0.02, 0.03, fit_txt, transform=ax.transAxes, fontsize=7.5, color="#334155", va="bottom")
...
fig.suptitle(
    "Per-task replay saturation — final metric vs labels replayed per step\n"
