...
def legend_handles() -> list[Line2D]:
    return [
        ...
        Line2D([], [], ls="-", color=GREEN, lw=1.8, label="single-task baseline (best of self-warm-restart ×10)"),
        Line2D([], [], ls="--", color=MUTED, lw=1.4, label="at-intro level (introduced within the 24-task sweep)"),
...
def load_single_task_baseline() -> dict[str, float]:
    """Best test primary over the 11 self-warm-restart iterations (results/warm_restart.csv)."""
    p = RES / "warm_restart.csv"
    best: dict[str, float] = {}
    if p.exists():
...
def draw_panel(ax, task: str, *, annotate_size: float = 7.5) -> tuple[str, str]:
    ...
    single = SINGLE_BASE.get(task)
    single_txt = f"{single:>7.3f}" if single is not None else "      -"
    ...
    if fit is not None:
        ...
        row = f"{task:24} {n_train:>7} {ceiling:>7.3f} {single_txt} {min(ys):>7.3f}..{max(ys):<7.3f} {k:>8,.0f} {n90:>8,.0f} {n90 / n_train:>10.1f}x"
    else:
        fit_txt = reason
        row = f"{task:24} {n_train:>7} {ceiling:>7.3f} {single_txt} {min(ys):>7.3f}..{max(ys):<7.3f} {reason}"
...
print(f"{'task':24} {'n_train':>7} {'intro':>7} {'single':>7} {'final(min..max)':>16} {'knee k':>8} {'n_90%':>8} {'90%/n_train':>11}")
print("-" * 118)
