...
function draw(){
  ...
  try{const s=document.getElementById("fig-vol-seeds");if(s)strips(s,[{name:"KMD",vals:D.runs.volume_old_kmd,col:C.alone},{name:"XenonPy classic",vals:D.runs.volume_xenonpy_classic,col:C.warm},{name:"XenonPy, no sum block",vals:D.runs.volume_xenonpy_nosum,col:C.frz},{name:"classic, plain z-score",vals:D.runs.volume_zscore_classic_discarded,col:C.xfer,sub:"discarded"}],"volume · R² of each run · single task, 5 seeds per arm · dots below the axis are off-scale","R²",{lo:0.4,hi:1.02});}catch(e){console.error("fig-vol-seeds failed",e);}
  ...
  try{(function(){const svg=document.getElementById("fig-curves");if(!svg)return;svg.textContent="";
    const runs=[["KMD (R² 0.61)",D.curves.volume_kmd_s2025,C.alone],["XenonPy classic, notebook transform (0.997)",D.curves.volume_xenonpy_classic_s2025,C.warm],["classic, plain z-score (0.43 → collapsed)",D.curves.volume_zscore_classic_s2025,C.xfer]];
