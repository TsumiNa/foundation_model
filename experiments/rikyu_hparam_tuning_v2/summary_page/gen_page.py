...
<li>{T("<b>Descriptor policy — decided (2026-09-11): KMD stays.</b> Its invertibility is what inverse design needs; the companion page shows it is scale-blind, so volume, total magnetisation and space group are reported as KMD-limited (R² 0.61 as cell volume against 0.997 with a scale-carrying descriptor) rather than switched to another descriptor.","<b>描述符策略——已定(2026-09-11):保留 KMD。</b>逆向设计需要它的可逆性;配套页面表明它对原胞尺度不敏感,因此 volume、总磁矩和空间群按“受 KMD 限制”报告(以晶胞体积计 R² 0.61,带尺度的描述符可到 0.997),而不是换描述符。","<b>記述子の方針 — 決定(2026-09-11):KMD を維持。</b>逆設計に必要なのはその可逆性であり、併設ページが示す通りスケールに盲目なので、volume・全磁化・空間群は「KMD の制約付き」として報告する(セル体積で R² 0.61、スケールを持つ記述子なら 0.997)。記述子は替えない。")}</li>
