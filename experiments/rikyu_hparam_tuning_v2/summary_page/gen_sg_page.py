...
setup_rows = [
    ...
    [T("loss", "损失", "損失"), T("plain cross-entropy", "普通交叉熵", "通常の交差エントロピー"), T(f"cross-entropy with sklearn-balanced class weights — applied to every classification head, unconditionally; here ×{w_min:.3f} for Fm-3m up to ×{w_max:.1f} for a 10-row group", f"带 sklearn balanced 类权重的交叉熵——对每个分类 head 无条件启用;这里 Fm-3m 为 ×{w_min:.3f},10 行的群为 ×{w_max:.1f}", f"sklearn balanced クラス重み付き交差エントロピー — 全分類ヘッドに無条件で適用。ここでは Fm-3m が ×{w_min:.3f}、10 行の群が ×{w_max:.1f}")],
