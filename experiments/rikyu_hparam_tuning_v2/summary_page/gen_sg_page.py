...
<li>{T("<b>Classification heads train on unweighted cross-entropy — material_type included.</b> The weights were designed for the five-class material_type head (99% of rows in one class), so that head was checked the same way, five seeds each in the pipeline: macro-F1 0.571 ± 0.031 with the weights (the recorded ceiling) against <b>0.834 ± 0.018</b> without; accuracy 0.9878 → 0.9978. Recall on the rare classes barely moves (IAC 0.95 → 0.86, IQC 0.84 → 0.84); their precision doubles (IAC 0.39 → 0.82, IQC 0.51 → 0.78), because the weighted head files 75 rows a run as IAC for 24 real ones. The weights do not even help the head they were built for. Consequence: every material_type transfer verdict (xfer +22%, frozen +27%, warm-start +22% over an alone of 0.571) was measured with the weights on in every arm and has to be re-measured against the 0.83 baseline. The other three heads, five seeds each, control = the recorded baselines: magnetic_ordering macro-F1 0.556 → 0.585, accuracy 0.749 → 0.829; is_metal unchanged (0.927 vs 0.927 — its two classes are balanced, so the weights are ≈ 1); is_gap_direct macro-F1 0.634 → 0.627 (a wash), accuracy 0.733 → 0.868. The price is minority recall where the minority is a real class: AFM (118 test rows) recall 0.56 → 0.14 while FM, FiM and NM move 0.56 → 0.70, 0.64 → 0.49, 0.84 → 0.94; direct-gap “yes” (620 rows) recall 0.79 → 0.25 with precision 0.31 → 0.53. Decision (2026-09-11): <code>class_weights = &quot;none&quot;</code> on every classification head; a screening use that needs direct-gap or AFM recall should move the decision threshold at inference, not retrain with weights.",
"<b>分类 head 改用不加权的交叉熵——material_type 也包括在内。</b>权重本是为五类的 material_type head(99% 的行属于一类)设计的,所以用同样方法在 pipeline 里各跑 5 个 seed 检验了它:带权重 macro-F1 0.571 ± 0.031(即记录在案的上限),不带权重 <b>0.834 ± 0.018</b>;准确率 0.9878 → 0.9978。稀有类的召回几乎不变(IAC 0.95 → 0.86,IQC 0.84 → 0.84),precision 翻倍(IAC 0.39 → 0.82,IQC 0.51 → 0.78),因为加权的 head 每次运行把 75 行判成 IAC,而真实只有 24 行。权重连它为之设计的 head 都没有帮到。后果:material_type 的所有迁移判定(xfer +22%、冻结 +27%、warm-start +22%,相对单独 0.571)在每个臂里都是带权重测的,必须对着 0.83 这个基线重测。其余三个 head 各 5 个 seed,对照为记录在案的基线:magnetic_ordering macro-F1 0.556 → 0.585、准确率 0.749 → 0.829;is_metal 不变(0.927 vs 0.927——两类本就均衡,权重 ≈ 1);is_gap_direct macro-F1 0.634 → 0.627(持平)、准确率 0.733 → 0.868。代价是少数类真实存在时它的召回:AFM(118 个测试行)召回 0.56 → 0.14,而 FM、FiM、NM 分别 0.56 → 0.70、0.64 → 0.49、0.84 → 0.94;直接带隙 “yes”(620 行)召回 0.79 → 0.25、precision 0.31 → 0.53。决定(2026-09-11):所有分类 head 用 <code>class_weights = &quot;none&quot;</code>;若筛选用途需要直接带隙或 AFM 的召回,在推理时移动判定阈值,而不是带权重重训。",
"<b>分類ヘッドは重みなしの交差エントロピーで学習する — material_type も含めて。</b>重みは5クラスの material_type ヘッド(行の 99% が1クラス)のために設計されたので、そのヘッドも同じ方法でパイプライン内で各 5 シード確認した:重みあり macro-F1 0.571 ± 0.031(記録された天井)、重みなし <b>0.834 ± 0.018</b>。正解率 0.9878 → 0.9978。稀なクラスの recall はほぼ動かず(IAC 0.95 → 0.86、IQC 0.84 → 0.84)、precision は倍増(IAC 0.39 → 0.82、IQC 0.51 → 0.78)。重み付きヘッドは実行あたり 75 行を IAC と判定するが、本物は 24 行しかないからだ。重みは設計対象のヘッドさえ助けていない。帰結:material_type の全転移判定(xfer +22%、凍結 +27%、ウォームスタート +22%、単独 0.571 に対して)は全アームで重みありで測られており、0.83 のベースラインに対して再測定が必要。残る3ヘッドは各 5 シード、対照は記録済みベースライン:magnetic_ordering macro-F1 0.556 → 0.585、正解率 0.749 → 0.829。is_metal は不変(0.927 対 0.927 — 2 クラスは元々均衡で重み ≈ 1)。is_gap_direct は macro-F1 0.634 → 0.627(互角)、正解率 0.733 → 0.868。代償は少数クラスが実在する場合のその recall:AFM(テスト 118 行)の recall 0.56 → 0.14、一方 FM・FiM・NM は 0.56 → 0.70、0.64 → 0.49、0.84 → 0.94。直接ギャップ “yes”(620 行)の recall 0.79 → 0.25、precision 0.31 → 0.53。決定(2026-09-11):全分類ヘッドで <code>class_weights = &quot;none&quot;</code>。直接ギャップや AFM の recall が必要なスクリーニング用途では、重み付きで再学習するのではなく推論時に判定閾値を動かす。")}</li>
