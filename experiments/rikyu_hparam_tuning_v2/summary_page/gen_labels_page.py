...
{TH("The six probed materials as numbers: each scheme, and the value in each dataset version","六种抽查材料的数值:各方案,以及两版数据集中的值","抜き取り6物質の数値:各スキームと、各データセット版の値")}{table([T("material","材料","物質"), T("GGA / GGA+U","GGA / GGA+U","GGA / GGA+U"), "r2SCAN", T("MP summary (mixed)","MP summary(混合)","MP summary(混合)"), T("dataset 2026-05-15","数据集 2026-05-15","データセット 2026-05-15"), T("dataset 2026-09-12","数据集 2026-09-12","データセット 2026-09-12")], probe_rows, style="max-width:900px")}
...
{TH("Single-task baselines on the 2026-09-12 dataset, with the old ceiling where one exists","2026-09-12 数据集上的单任务基线,有旧上限的附旧上限","2026-09-12 データセットでの単独学習基準、旧天井がある場合は併記")}{table([T("task","任务","タスク"), T("group","类别","区分"), T("metric","指标","指標"), T("test rows","测试行数","テスト行数"), T("mean ± sd, 5 seeds","均值 ± sd,5 seed","平均 ± sd、5シード"), "MAE", T("2026-05-15 ceiling","2026-05-15 上限","2026-05-15 天井"), "Δ"], rows, cls="tablebox")}
<div class="col" style="margin-top:16px">
...
<figure style="margin-top:14px">{FH("Non-null rows per column, 2026-05-15 vs 2026-09-12","逐列非空行数,2026-05-15 vs 2026-09-12","列ごとの非欠損行数、2026-05-15 vs 2026-09-12")}<div class="figbox"><svg id="fig-cols" width="960" height="660" role="img" aria-label="Non-null rows per column before and after"></svg></div>
...
<li>{T("<b>Re-run the transfer stages on the 2026-09-12 dataset</b> with that set — the 2×2 on the old data is closed and warm-start is the method; volume enters through its per-atom form, or with a scale-aware descriptor.","<b>在 2026-09-12 数据集上重跑迁移阶段</b>——旧数据上的 2×2 已收尾,迁移方式定为 warm-start;volume 以每原子形式进入,或改用能感知尺度的描述符。","<b>2026-09-12 データセットで転移段階を再実行</b>する — 旧データでの 2×2 は完了し、方式はウォームスタートに決まった。volume は原子あたりの形で、あるいはスケール認識記述子で入れる。")}</li>
