...
the rebuilt dataset follows, and the update brief for the 2026-09-11 dataset. English is the source
...
# ---------------- header ----------------
out.append(f'''<header>
<p class="eyebrow">{T("Continual multi-task pretraining · Materials Project labels · findings and data update, 2026-09-11","连续多任务预训练 · Materials Project 标签 · 发现与数据更新,2026-09-11","継続的マルチタスク事前学習 · Materials Project ラベル · 知見とデータ更新、2026-09-11")}</p>
...
<figure>{FH("Single-task R² of the six Materials Project tasks on the 2026-05-15 labels","六个 Materials Project 任务在 2026-05-15 标签上的单任务 R²","2026-05-15 ラベルでの Materials Project 6タスクの単独学習 R²")}<div class="figbox"><svg id="fig-ceil" width="960" height="320" role="img" aria-label="Single-task R² of the six MP tasks"></svg></div>
...
{TH("The six tasks: training rows, R² and spread over seeds","六个任务:训练行数、R² 与 seed 间离散","6タスク:訓練行数、R²、シード間のばらつき")}{table([T("task","任务","タスク"), "N train", "R²", "sd"], rows, style="max-width:520px")}
</section>''')
...
<figure>{FH("Energy per atom of six known materials under each DFT scheme, and where each dataset version falls","六种已知材料在各 DFT 方案下的每原子能量,以及两版数据集的值各落在哪里","既知6物質の各DFTスキームでの原子あたりエネルギーと、各データセット版の値の位置")}<div class="figbox"><div class="legend"><span><i class="sw" style="background:var(--warm)"></i> {T("GGA / GGA+U","GGA / GGA+U","GGA / GGA+U")}</span><span><i class="sw" style="background:var(--surface);border:2px solid var(--frz)"></i> r2SCAN</span><span><i class="sw sq" style="background:var(--xfer)"></i> {T("dataset 2026-05-15","数据集 2026-05-15","データセット 2026-05-15")}</span><span><i class="sw" style="background:none;border:1.5px dashed var(--warm)"></i> {T("dataset 2026-09-11","数据集 2026-09-11","データセット 2026-09-11")}</span></div>
<svg id="fig-probe" width="960" height="360" role="img" aria-label="Energy per atom of six materials under each scheme"></svg></div>
{CAP("Six materials queried from the Materials Project API on 2026-09-11. The old dataset value (square) sits on the r2SCAN value wherever MP has an r2SCAN calculation, and on the GGA value where it does not (NaCl). The rebuilt dataset (dashed ring) sits on GGA / GGA+U for all of them.",
"2026-09-11 从 Materials Project API 查询的六种材料。旧数据集的值(方块)在 MP 有 r2SCAN 计算的地方落在 r2SCAN 值上,没有的地方落在 GGA 值上(NaCl)。重建后的数据集(虚线圈)全部落在 GGA / GGA+U 上。",
"2026-09-11 に Materials Project API から取得した6物質。旧データセットの値(四角)は MP に r2SCAN 計算がある物質では r2SCAN 値に、ない物質(NaCl)では GGA 値に一致する。再構築データセット(破線の輪)は全て GGA / GGA+U 上にある。")}</figure>
{TH("The six probed materials as numbers: each scheme, and the value in each dataset version","六种抽查材料的数值:各方案,以及两版数据集中的值","抜き取り6物質の数値:各スキームと、各データセット版の値")}{table([T("material","材料","物質"), T("GGA / GGA+U","GGA / GGA+U","GGA / GGA+U"), "r2SCAN", T("MP summary (mixed)","MP summary(混合)","MP summary(混合)"), T("dataset 2026-05-15","数据集 2026-05-15","データセット 2026-05-15"), T("dataset 2026-09-11","数据集 2026-09-11","データセット 2026-09-11")], probe_rows, style="max-width:900px")}
...
<figure>{FH("Mixed-scheme energy minus GGA energy, for every material that has both","每个同时有两种口径的材料:混合方案能量减去 GGA 能量","両スキームを持つ全物質での、混合スキーム − GGA エネルギー")}<div class="figbox"><svg id="fig-mixed" width="960" height="300" role="img" aria-label="Mixed-scheme minus GGA energy over all materials"></svg></div>
...
{TH("Two consistency checks on the old final_energy label","对旧 final_energy 标签的两项一致性检验","旧 final_energy ラベルの2つの整合性検証")}{table([T("check","检验","検証"), T("result","结果","結果"), T("reading","解读","解釈")], [
...
<figure>{FH("The final_energy label before and after the rebuild, same rows","重建前后的 final_energy 标签,同一批行","再構築前後の final_energy ラベル、同じ行")}<div class="figbox"><div class="legend"><span><i class="sw sq" style="background:var(--xfer)"></i> {T("2026-05-15 label","2026-05-15 标签","2026-05-15 ラベル")}</span><span><i class="sw sq" style="background:var(--warm)"></i> {T("GGA / GGA+U label","GGA / GGA+U 标签","GGA / GGA+U ラベル")}</span></div>
...
<figure>{FH("final_energy R² of each seed, old label vs GGA / GGA+U label","final_energy 每个 seed 的 R²,旧标签 vs GGA / GGA+U 标签","final_energy の各シードの R²、旧ラベル vs GGA / GGA+U ラベル")}<div class="figbox"><div class="legend"><span>● {T("one seed","一个 seed","1シード")} · ▬ {T("median","中位数","中央値")}</span></div><svg id="fig-fe-seeds" width="960" height="300" role="img" aria-label="final_energy R² per seed before and after"></svg></div>
...
<figure>{FH("Atoms per cell in the compositions the model receives","模型收到的组成中的原胞原子数分布","モデルが受け取る組成の単位胞原子数の分布")}<div class="figbox"><svg id="fig-atoms" width="960" height="280" role="img" aria-label="Atoms per cell distribution"></svg></div>
...
{TH("Linear ridge regression on the true targets, by descriptor","对真实目标的线性岭回归,按描述符","真の目標に対する線形リッジ回帰、記述子別")}{table([T("target","目标","目標"), "KMD", T("XenonPy classic","XenonPy classic","XenonPy classic"), T("without the sum block","去掉 sum 块","sum ブロックなし")], [
...
{TH("Single-task network R² by descriptor, three tasks","单任务网络的 R²,按描述符,三个任务","単独学習ネットワークの R²、記述子別、3タスク")}{table([T("task · single-task network, 5 seeds","任务 · 单任务网络,5 seed","タスク · 単独学習ネットワーク、5シード"), "KMD", T("XenonPy classic (vs KMD)","XenonPy classic(vs KMD)","XenonPy classic(vs KMD)"), T("without the sum block (vs KMD)","去掉 sum 块(vs KMD)","sum ブロックなし(vs KMD)")], desc_rows, style="max-width:900px")}
<figure style="margin-top:14px">{FH("volume R² of each seed with three descriptors: KMD, XenonPy classic, and classic without the sum block","volume 每个 seed 的 R²,三种描述符:KMD、XenonPy classic、去掉 sum 块的 classic","volume の各シードの R²、3つの記述子:KMD、XenonPy classic、sum ブロックなしの classic")}<div class="figbox"><div class="legend"><span>● {T("one seed","一个 seed","1シード")} · ▬ {T("median","中位数","中央値")}</span></div><svg id="fig-vol-seeds" width="960" height="300" role="img" aria-label="volume R² per seed by descriptor"></svg></div>
...
<figure>{FH("final_energy and dos_density R² of each seed with the same three descriptors","final_energy 与 dos_density 每个 seed 的 R²,同样三种描述符","final_energy と dos_density の各シードの R²、同じ3つの記述子")}<div class="figbox"><div class="legend"><span>● {T("one seed","一个 seed","1シード")} · ▬ {T("median","中位数","中央値")}</span></div><svg id="fig-other-seeds" width="960" height="300" role="img" aria-label="final_energy and dos_density R² per seed by descriptor"></svg></div>
...
<figure>{FH("volume — training and validation loss over epochs, KMD vs XenonPy classic","volume——训练与验证损失随 epoch 的变化,KMD vs XenonPy classic","volume — エポックごとの学習・検証損失、KMD vs XenonPy classic")}<div class="figbox"><div class="legend"><span><i class="sw" style="background:var(--alone)"></i> KMD</span><span><i class="sw" style="background:var(--warm)"></i> {T("XenonPy classic","XenonPy classic","XenonPy classic")}</span></div>
...
# ---------------- 4 what it voids ----------------
void_rows = [[t.replace("_", " "), neg(f"{a:+.1f}%"), neg(f"{b:+.1f}%"), neg(f"{c:+.1f}%"), T("void — label / descriptor artefact; re-measure on the 2026-09-11 dataset", "作废——标签 / 描述符假象;在 2026-09-11 数据集上重测", "無効 — ラベル / 記述子の人工物。2026-09-11 データセットで再測定")] for t, a, b, c in D["transfer_void"]]
...
def table_raw(head, rows, style=""):
    ...
    return f'<div class="tablebox prose"{(" style=" + chr(34) + style + chr(34)) if style else ""}><table><thead><tr>{th}</tr></thead><tbody>{body}</tbody></table></div>'
...
{TH("Transfer verdicts on the 2026-05-15 labels: which are void, which stand","2026-05-15 标签上的迁移判定:哪些作废,哪些成立","2026-05-15 ラベルでの転移判定:無効なもの、有効なもの")}{table_raw([T("task","任务","タスク"), T("xfer vs alone","xfer vs 单独","xfer vs 単独"), T("frozen vs alone","冻结 vs 单独","凍結 vs 単独"), T("warm-start vs alone","warm-start vs 单独","ウォームスタート vs 単独"), T("status","状态","状態")], strip_td(void_rows + keep_rows), style="max-width:1000px")}
...
{TH("The added properties: what each is, how Materials Project computes it, its unit, and our label","新增属性:是什么、Materials Project 怎么算、单位、我们的标签","追加した物性:何か、Materials Project の計算方法、単位、本研究のラベル")}{table(head, rows, cls="tablebox prose")}
</section>'''
...
{TH("Single-task baselines on the 2026-09-11 dataset, with the old ceiling where one exists","2026-09-11 数据集上的单任务基线,有旧上限的附旧上限","2026-09-11 データセットでの単独学習基準、旧天井がある場合は併記")}{table([T("task","任务","タスク"), T("group","类别","区分"), T("metric","指标","指標"), T("test rows","测试行数","テスト行数"), T("mean ± sd, 5 seeds","均值 ± sd,5 seed","平均 ± sd、5シード"), "MAE", T("2026-05-15 ceiling","2026-05-15 上限","2026-05-15 天井"), "Δ"], rows, cls="tablebox")}
<div class="col" style="margin-top:16px">
...
{P('<b>Space group, revised (2026-09-11).</b> The 0.20 / 0.24 above is not what composition can do: a follow-up investigation (<a href="https://claude.ai/code/artifact/fd292ecf-e1d0-44db-9514-d8c3d2ee31d1">Space group: 0.24 here, 0.60 in the paper</a>) traced it to the pipeline’s class-balanced loss (−21 points on 151 classes) and to KMD’s blindness to the number of atoms in the cell (−11 points). With the weights off the same head reaches accuracy 0.465 / macro-F1 0.313 (5 seeds), and 0.557 / 0.400 with the XenonPy classic descriptor; Pnma and P2<sub>1</sub>/c are then recalled at 45% and 64%, so the sentence about high-symmetry families only is withdrawn. The same weights were on for every classification baseline above; without them (5 seeds each) material_type reaches macro-F1 0.834 (from 0.571), magnetic_ordering 0.585 / accuracy 0.829 (from 0.556 / 0.749), is_metal is unchanged, and is_gap_direct trades minority recall for accuracy (0.733 → 0.868, macro-F1 level). All classification heads now train with <code>class_weights = &quot;none&quot;</code>.',
'<b>空间群,修订(2026-09-11)。</b>上面的 0.20 / 0.24 并不代表组成能做到的水平:后续调查(<a href="https://claude.ai/code/artifact/fd292ecf-e1d0-44db-9514-d8c3d2ee31d1">空间群:这里 0.24,论文里 0.60</a>)把它归因于 pipeline 的类平衡损失(151 类上 −21 个百分点)和 KMD 看不见原胞原子数(−11 个百分点)。关掉权重后同一个 head 达到准确率 0.465 / macro-F1 0.313(5 seed),换 XenonPy classic 描述符后为 0.557 / 0.400;届时 Pnma 和 P2<sub>1</sub>/c 的召回率分别为 45% 和 64%,所以“只能认出高对称家族”那句话收回。上面所有分类基线都是带权重跑的;去掉权重后(各 5 seed)material_type 的 macro-F1 达到 0.834(原 0.571),magnetic_ordering 为 0.585 / 准确率 0.829(原 0.556 / 0.749),is_metal 不变,is_gap_direct 以少数类召回换准确率(0.733 → 0.868,macro-F1 持平)。现在所有分类 head 都用 <code>class_weights = &quot;none&quot;</code> 训练。',
'<b>空間群、改訂(2026-09-11)。</b>上の 0.20 / 0.24 は組成でできることを表していない:追跡調査(<a href="https://claude.ai/code/artifact/fd292ecf-e1d0-44db-9514-d8c3d2ee31d1">空間群:ここでは 0.24、論文では 0.60</a>)はパイプラインのクラス平衡損失(151 クラスで −21 ポイント)と、KMD が単位胞の原子数を見られないこと(−11 ポイント)に原因を突き止めた。重みを切ると同じヘッドが正解率 0.465 / macro-F1 0.313(5 シード)、XenonPy classic 記述子では 0.557 / 0.400 に達し、Pnma と P2<sub>1</sub>/c の recall は 45% と 64% になるので、高対称族しか分からないという文は撤回する。上の分類ベースラインはすべて重みありで実行された。重みなし(各 5 シード)では material_type が macro-F1 0.834(0.571 から)、magnetic_ordering が 0.585 / 正解率 0.829(0.556 / 0.749 から)、is_metal は不変、is_gap_direct は少数クラスの recall と引き換えに正解率が上がる(0.733 → 0.868、macro-F1 は互角)。全分類ヘッドは今後 <code>class_weights = &quot;none&quot;</code> で学習する。', "callout")}
</div>
<figure style="margin-top:14px">{FH("Observed vs predicted for every regression task on the rebuilt dataset, one seed each","重建数据集上每个回归任务的观测值 vs 预测值,各一个 seed","再構築データセットでの各回帰タスクの観測値 vs 予測値、各1シード")}<div class="figbox"><div class="legend"><span><i class="sw" style="background:var(--warm)"></i> {T("relabelled task","标签更新的任务","ラベル更新タスク")}</span><span><i class="sw" style="background:var(--frz)"></i> {T("added task","新增任务","新規タスク")}</span><span style="color:var(--muted)">{T("2,000 test rows sampled per task · seed 2025 · normalised scale · dashed = y = x","每任务抽样 2,000 个测试行 · seed 2025 · 归一化尺度 · 虚线为 y = x","タスクごとにテスト行 2,000 を抽出 · シード 2025 · 正規化スケール · 破線は y = x")}</span></div>
...
<figure>{FH("Confusion matrices of magnetic_ordering, is_metal and is_gap_direct","magnetic_ordering、is_metal、is_gap_direct 的混淆矩阵","magnetic_ordering、is_metal、is_gap_direct の混同行列")}<div class="figbox"><svg id="fig-cm" width="960" height="300" role="img" aria-label="Confusion matrices of the classification tasks"></svg></div>
...
<figure>{FH("space_group — confusion matrix over the twelve most common groups, the rest folded into “other”","space_group——最常见 12 个群的混淆矩阵,其余并入“other”","space_group — 最も多い12群の混同行列、残りは“other”にまとめる")}<div class="figbox"><svg id="fig-sg" width="960" height="620" role="img" aria-label="Space-group confusion over the twelve most common groups"></svg></div>
...
<figure>{FH("space_group — F1 of each group against how many entries the group has","space_group——每个群的 F1 对该群的条目数","space_group — 各群の F1 と、その群の項目数")}<div class="figbox"><svg id="fig-sg-f1" width="960" height="330" role="img" aria-label="Per-class F1 against class size for the space-group task"></svg></div>
...
out.append(f'''<section class="section">{H2("6 · Update brief", "Dataset 2026-09-11: what changed", "数据集 2026-09-11:变了什么", "データセット 2026-09-11:何が変わったか")}
...
<div class="col">{P("Files: <code>data/qc_ac_te_mp_dos_reformat_20260912.pd.parquet</code>, <code>data/preprocessing_objects_20260912.pkl.z</code>, <code>data/qc_ac_te_mp_dos_reformat_20260912_CHANGES.md</code>; built by <code>data/data/scripts/rebuild_mp_gga_20260912.py</code> from API pulls made on 2026-09-11. The 2026-05-15 files are untouched.",
"文件:<code>data/qc_ac_te_mp_dos_reformat_20260912.pd.parquet</code>、<code>data/preprocessing_objects_20260912.pkl.z</code>、<code>data/qc_ac_te_mp_dos_reformat_20260912_CHANGES.md</code>;由 <code>data/data/scripts/rebuild_mp_gga_20260912.py</code> 从 2026-09-11 的 API 拉取生成。2026-05-15 的文件未动。",
"ファイル:<code>data/qc_ac_te_mp_dos_reformat_20260912.pd.parquet</code>、<code>data/preprocessing_objects_20260912.pkl.z</code>、<code>data/qc_ac_te_mp_dos_reformat_20260912_CHANGES.md</code>。<code>data/data/scripts/rebuild_mp_gga_20260912.py</code> が 2026-09-11 の API 取得から生成。2026-05-15 のファイルは未変更。")}</div>
{TH("Every column the rebuild touched: its source and its row count","重建涉及的每一列:来源与行数","再構築で触れた全列:出典と行数")}{table([T("column(s)","列","列"), T("source in the rebuilt dataset","重建数据集中的来源","再構築データセットでの出典"), T("MP rows","MP 行数","MP 行数")], newcol_rows, cls="tablebox prose")}
<figure style="margin-top:14px">{FH("Non-null rows per column, 2026-05-15 vs 2026-09-11","逐列非空行数,2026-05-15 vs 2026-09-11","列ごとの非欠損行数、2026-05-15 vs 2026-09-11")}<div class="figbox"><svg id="fig-cols" width="960" height="660" role="img" aria-label="Non-null rows per column before and after"></svg></div>
...
{TH("Old vs new values, column by column: quantiles of the ratio or difference","新旧值逐列对比:比值或差值的分位数","新旧の値の列ごとの比較:比または差の分位数")}{table([T("column","列","列"), T("comparison","比较","比較"), "1%", T("median","中位数","中央値"), "99%", T("|Δ| > 0.05","|Δ| > 0.05","|Δ| > 0.05"), T("rows","行数","行数")], val_rows, style="max-width:1000px")}
<div class="col" style="margin-top:16px">
...
<li>{T("<b>Re-run the transfer stages on the 2026-09-11 dataset</b> with that set — the 2×2 on the old data is closed and warm-start is the method; volume enters through its per-atom form, or with a scale-aware descriptor.","<b>在 2026-09-11 数据集上重跑迁移阶段</b>——旧数据上的 2×2 已收尾,迁移方式定为 warm-start;volume 以每原子形式进入,或改用能感知尺度的描述符。","<b>2026-09-11 データセットで転移段階を再実行</b>する — 旧データでの 2×2 は完了し、方式はウォームスタートに決まった。volume は原子あたりの形で、あるいはスケール認識記述子で入れる。")}</li>
<li>{T("<b>Descriptor policy — decided (2026-09-11): KMD stays</b> for its invertibility. volume, total magnetisation and space group are reported with the KMD caveat (the label needs a cell scale the descriptor cannot see) rather than moved to another descriptor; classification heads train with <code>class_weights = &quot;none&quot;</code>.","<b>描述符策略——已定(2026-09-11):保留 KMD</b>,因为需要可逆性。volume、总磁矩和空间群带着 KMD 的说明报告(标签依赖描述符看不见的原胞尺度),不再换描述符;分类 head 使用 <code>class_weights = &quot;none&quot;</code>。","<b>記述子の方針 — 決定(2026-09-11):KMD を維持</b>、可逆性のため。volume・全磁化・空間群は KMD の注記付きで報告し(ラベルが記述子に見えない単位胞スケールを必要とする)、記述子は替えない。分類ヘッドは <code>class_weights = &quot;none&quot;</code> で学習する。")}</li>
</ol></div>
</section>
{props_section()}
{baselines_section()}
<footer><p>{T("Data: summary/mp_labels_page_data.json (per-seed results, curves, API comparisons, dataset statistics); runs stA_*, stM_final_energy_*, stXc_* and stXn_* on RIKYU; MP API queries of 2026-09-11. Figure labels stay in English in every language.",
"数据:summary/mp_labels_page_data.json(逐 seed 结果、曲线、API 比对、数据集统计);RIKYU 上的 stA_*、stM_final_energy_*、stXc_*、stXn_*;2026-09-11 的 MP API 查询。图内标签在各语言下均保留英文。",
"データ:summary/mp_labels_page_data.json(シードごとの結果、曲線、API 比較、データセット統計)。RIKYU 上の stA_*、stM_final_energy_*、stXc_*、stXn_*。2026-09-11 の MP API 照会。図中のラベルはどの言語でも英語のまま。")}</p></footer>
</div>
<script>
{LANGJS}
</script>
