...
def baselines_section():
    ...
    adopted = EXP / "summary" / "space_group_baseline_adopted.json"
    if adopted.exists():
        sg = json.load(open(adopted)); B["per_task"] = [sg if r["task"] == "space_group" else r for r in B["per_task"]]
    rows = []
    for r in B["per_task"]:
...
{P("<b>Added tasks.</b> Band gap 0.88, CBM 0.88, VBM 0.92, is-metal macro-F1 0.93, bulk modulus 0.93, refractive index 0.89 and shear modulus 0.79 are solid new tasks. Poisson ratio (0.31), universal anisotropy (0.35) and reaction energy (0.34) carry little composition signal, and piezoelectric maximum (196 test rows) none at all; they should enter the multi-task set only as deliberately hard or low-data tasks, or not at all. Space group (151 classes) reaches macro-F1 0.31 and accuracy 0.47 (5 seeds): composition identifies the high-symmetry families (Fm-3m recalled 86%, I4/mmm and Cmcm 63%, P-62m 60%) and the low-symmetry groups only partly (Pnma 45%, P2₁/c 64%, C2/c 22%, P-1 16%) — those are decided by structure more than by stoichiometry. Part of the gap is the descriptor: KMD works on atomic fractions and cannot see the number of atoms in the cell; with XenonPy's classic descriptor, which carries it, the same head reaches accuracy 0.56 / macro-F1 0.40.",
"<b>新增任务。</b>Band gap 0.88、CBM 0.88、VBM 0.92、is-metal macro-F1 0.93、bulk modulus 0.93、折射率 0.89、shear modulus 0.79 是扎实的新任务。泊松比(0.31)、通用各向异性(0.35)和反应能(0.34)组成信号很弱,压电最大值(196 个测试行)完全学不到;它们只应作为刻意设置的困难 / 小数据任务进入多任务集,或者不进。空间群(151 类)macro-F1 0.31、准确率 0.47(5 个 seed):组成能认出高对称家族(Fm-3m 召回 86%,I4/mmm 与 Cmcm 63%,P-62m 60%),低对称群只能部分认出(Pnma 45%,P2₁/c 64%,C2/c 22%,P-1 16%)——它们更多由结构而非化学计量决定。差距的一部分来自描述符:KMD 只看原子分数,看不见原胞里的原子数;换成带原子数的 XenonPy classic 描述符后,同一个 head 达到准确率 0.56 / macro-F1 0.40。",
"<b>新規タスク。</b>バンドギャップ 0.88、CBM 0.88、VBM 0.92、is-metal macro-F1 0.93、体積弾性率 0.93、屈折率 0.89、剪断弾性率 0.79 は堅実な新規タスク。ポアソン比(0.31)、普遍異方性(0.35)、反応エネルギー(0.34)は組成の信号が弱く、圧電最大値(テスト行 196)は全く学習できない。これらは意図的な難タスク・少データタスクとしてのみマルチタスク集合に入れるか、入れない。空間群(151クラス)は macro-F1 0.31、正解率 0.47(5 シード):組成から高対称族は分かり(Fm-3m の recall 86%、I4/mmm と Cmcm 63%、P-62m 60%)、低対称群は部分的にしか分からない(Pnma 45%、P2₁/c 64%、C2/c 22%、P-1 16%)— 化学量論より構造が決めるものだからだ。差の一部は記述子による:KMD は原子分率だけを見て単位胞の原子数を見られない。原子数を含む XenonPy classic 記述子では同じヘッドが正解率 0.56 / macro-F1 0.40 に達する。", "callout")}
</div>
...
<li>{T("<b>Descriptor policy — decided (2026-09-11): KMD stays</b> for its invertibility. volume, total magnetisation and space group are reported with the KMD caveat (the label needs a cell scale the descriptor cannot see) rather than moved to another descriptor.","<b>描述符策略——已定(2026-09-11):保留 KMD</b>,因为需要可逆性。volume、总磁矩和空间群带着 KMD 的说明报告(标签依赖描述符看不见的原胞尺度),不再换描述符。","<b>記述子の方針 — 決定(2026-09-11):KMD を維持</b>、可逆性のため。volume・全磁化・空間群は KMD の注記付きで報告し(ラベルが記述子に見えない単位胞スケールを必要とする)、記述子は替えない。")}</li>
</ol></div>
</section>
{props_section()}
{baselines_section()}
<footer><p>{T("Data: summary/mp_labels_page_data.json (per-seed results, curves, API comparisons, dataset statistics); runs stA_*, stM_final_energy_*, stXc_*, stXn_* and stW_space_group_* on RIKYU; MP API queries of 2026-09-11. Figure labels stay in English in every language.",
