...
{P('<b>Space group, revised (2026-09-11).</b> The 0.20 / 0.24 above is not what composition can do: a follow-up investigation (<a href="https://claude.ai/code/artifact/fd292ecf-e1d0-44db-9514-d8c3d2ee31d1">Space group: 0.24 here, 0.60 in the paper</a>) traced it to the pipeline’s class-balanced loss (−21 points on 151 classes) and to KMD’s blindness to the number of atoms in the cell (−11 points). With the weights off the same head reaches accuracy 0.465 / macro-F1 0.313 (5 seeds), and 0.557 / 0.400 with the XenonPy classic descriptor; Pnma and P2<sub>1</sub>/c are then recalled at 45% and 64%, so the sentence about high-symmetry families only is withdrawn.',
'<b>空间群,修订(2026-09-11)。</b>上面的 0.20 / 0.24 并不代表组成能做到的水平:后续调查(<a href="https://claude.ai/code/artifact/fd292ecf-e1d0-44db-9514-d8c3d2ee31d1">空间群:这里 0.24,论文里 0.60</a>)把它归因于 pipeline 的类平衡损失(151 类上 −21 个百分点)和 KMD 看不见原胞原子数(−11 个百分点)。关掉权重后同一个 head 达到准确率 0.465 / macro-F1 0.313(5 seed),换 XenonPy classic 描述符后为 0.557 / 0.400;届时 Pnma 和 P2<sub>1</sub>/c 的召回率分别为 45% 和 64%,所以“只能认出高对称家族”那句话收回。',
'<b>空間群、改訂(2026-09-11)。</b>上の 0.20 / 0.24 は組成でできることを表していない:追跡調査(<a href="https://claude.ai/code/artifact/fd292ecf-e1d0-44db-9514-d8c3d2ee31d1">空間群:ここでは 0.24、論文では 0.60</a>)はパイプラインのクラス平衡損失(151 クラスで −21 ポイント)と、KMD が単位胞の原子数を見られないこと(−11 ポイント)に原因を突き止めた。重みを切ると同じヘッドが正解率 0.465 / macro-F1 0.313(5 シード)、XenonPy classic 記述子では 0.557 / 0.400 に達し、Pnma と P2<sub>1</sub>/c の recall は 45% と 64% になるので、高対称族しか分からないという文は撤回する。', "callout")}
</div>
