...
全部阶段完成并验证。`v1-complete` 标签存在，v1 产物未被重跑或修改（v1 的 Stage C 仅被**只读地
重新评分**，输出写入 v2 的 `summary/stage_c_v1_rescored.json`）。
...
每项都带 seed 数与噪声口径。噪声带 = 单run 极差（与 v1 口径一致）；判定用差值的标准误。

| 阶段 | 配置 | 指标 | 均值 | seed 数 | 可分辨阈值 | 是否可分辨 |
|---|---|---|---|---|---|---|
| Stage 0 锚点 | 未调 + 0.3.2 镜像 | probe6 相对分 | 参考点 | 9 | σ=2.05% | — |
| Stage 0 | v1 编码器 vs 未调 | 同上 | −0.83% | 9 | 2SE 2.73% | **否** |
...
### Stage C′ 四臂（+ v1 四臂，共 10 个）

| 臂 | mean R² | big | mid | small |
|---|---|---|---|---|
| v2 `c2_top1_cons` | **0.7274** | +0.0206 | −0.0119 | −0.0283 |
| v2 `c2_top3` | 0.7262 | +0.0109 | −0.0071 | −0.0194 |
| v2 `c2_top1` | 0.7229 | +0.0244 | −0.0071 | −0.0243 |
| v2 `c2_top2` | 0.7225 | +0.0197 | −0.0067 | −0.0076 |
| v2 `c2_base_cons` | 0.7189 | +0.0209 | +0.0004 | −0.0218 |
| v1 `c_tuned_cons` | 0.7186 | +0.0301 | −0.0035 | −0.0190 |
| **v2 `c2_base`（仅升级）** | **0.7155** | +0.0214 | +0.0065 | −0.0286 |
| v1 `c_tuned` | 0.7110 | +0.0248 | +0.0068 | +0.0086 |
| v1 `c_base` | 0.6784 | +0.0477 | +0.0423 | +0.0505 |
| v1 `c_base_cons` | 0.6770 | +0.0526 | +0.0368 | +0.0897 |
...
| 从 → 到 | Δ mean R² | 相对 | 隔离出什么 |
|---|---|---|---|
| `c_base` → `c2_base` | **+0.0371** | **+5.47%** | 仅 PR #45 调度器修复，完全不调参 |
| `c2_base` → `c2_top1` | +0.0074 | +1.03% | 在此之上的调参 |
| `c_base` → `c_tuned` | +0.0326 | +4.81% | v1 报告的收益（两者纠缠） |

上游修复是其后调参的 **5.0 倍**，也大于 v1 报告的全部收益。**未调参模型跑在修好的代码上
（0.7155）已超过 v1 调过参的臂（0.7110）。** v1 无法自己发现这点——它没有"未调参 + 当前代码"
的对照臂。
...
**3. v1 的一条已发布结论必须撤回。** v1 调参臂的负 deficit（mid −0.021 / small −0.028，被读作
"超过单任务天花板"）是天花板测低造成的假象。旧天花板测于 PR #45 之前，在 23 个回归/KR 任务里
**17 个偏低、平均 +0.0275**，且**不是常数**（seebeck 低 0.104，dielectric_ionic 反而高 0.017），
无法用偏移量修正。重算后是 **+0.007 / +0.009**——没有任何一项超过天花板。
...
3. **把"什么都不改"作为正式配置放进网格**：只占 24 个格点里的 1 个，却让"head 不需要调"成为
   排名结果而非论证。
4. **继承的基线必须在当前régime重测**：这一条撤回了 v1 一个已发布结论。
5. **凡是"在当前最优上"做的实验，最优一变就必须重做**：本轮踩了两次（早停、24 任务天花板）。
6. **分组均值会掩盖相消**：v1 的 small 组 −0.019 实为 +0.060 与 −0.022 相抵。
...
| 合并报告 | `results/REPORT_v1v2_20260828.md`（gitignore） |
| 演示文稿（25 页） | `results/REPORT_v1v2_20260828.pptx`（gitignore） |
...
| 方法学记录 | `NOTES.md` |
| v1 报告 | `experiments/rikyu_hparam_tuning/results/REPORT_20260826.md` |
