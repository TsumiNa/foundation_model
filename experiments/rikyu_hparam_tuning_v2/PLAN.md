...
> | 归你 | 不归你 |
> |---|---|
> | 跑完 Stage 0 / A' / B' / C' | 重跑或改动 v1 的任何产物 |
> | 产出 v2 的分析产物（JSON / 图） | —— |
> | **撰写 v1+v2 的合并报告与 PPT** | |
> | **收口 PR #41** | |
> | 记录偏离计划之处与异常 | |
...
> 有几条错误判断至今仍留在 git 历史的 commit message 里，不读交接文档会被误导。
>
> §9 的信号协议仍然要执行（打 tag、写 HANDOFF.md），但收信号的是**用户**，
> 用于确认 v2 收尾完成，而不是移交给另一个 session。
...
v1 与 v2 在**两个不同的 session** 里。下一个 session（也就是你）看不到 v1 的对话，
只能看到它留下的**文件与 git 状态** —— 这正是 `SESSION_HANDOVER.md` 存在的原因。

同理，你完成后也要留下同样可被轮询的痕迹：用户（以及任何后续 session）靠 tag 与
`HANDOFF.md` 判断 v2 是否结束，而不是靠一句口头汇报。
...
拿到 v2 全部结果后，由你完成跨 v1/v2 的综述：

1. 读 `SESSION_HANDOVER.md`（v1 的全部结论、置信度、已知失效项、踩过的坑）
2. 读 v1 的封存产物：`experiments/rikyu_hparam_tuning/results/REPORT_*.md` 与 `.pptx`
3. 合并四个参照点，回答三个问题：
   - **升级值多少**：v1 `c_base`（未调+坏调度器）→ v2 Stage 0 锚点（未调+好调度器）
   - **调参值多少**：v2 Stage 0 锚点 → v2 `c2_top1`
   - **probe 排名是否迁移**：v2 `c2_top1/2/3` 在 24 任务上的排序，是否与 probe 上一致
     ← 这是 v1 暴露出的最大不确定性，v2 专门为它设计了多 arm
4. 出合并报告与 PPT（可复用 v1 的 `build_report_pptx.py`，它已经是数据驱动的）
5. 收口 PR #41

**报告口径沿用 §6，不要放松** —— 尤其是"一切排序都对着噪声带说话"和"指标依赖必须披露"。
