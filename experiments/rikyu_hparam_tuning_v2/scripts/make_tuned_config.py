...
def patch_scalar(text: str, section: str, key: str, value: str) -> tuple[str, str]:
    """Set ``key = value`` inside ``[section]``, creating either if absent.

    v1's generator required both to exist already, which was right for v1: every knob it tuned was
    a key its control config already spelled out. v2 tunes ``[training.scheduler]``, a section
    v1's configs do not contain at all — before PR #45 those knobs were inert, so nobody wrote
    them down. Creating the section here is what lets the tuned stage-C' arm stay a patch of the
    control's own file instead of a separately authored one, which is the property that makes the
    two arms a like-for-like comparison.
    """
    sec = re.search(rf"^\[{re.escape(section)}\]\s*$", text, re.M)
    if not sec:
        # Insert the new section directly after its parent's block when there is one, so
        # [training.scheduler] lands next to [training] rather than at the end of the file.
        parent = section.rpartition(".")[0]
        anchor = re.search(rf"^\[{re.escape(parent)}\]\s*$", text, re.M) if parent else None
        if anchor:
            end = re.search(r"^\[", text[anchor.end():], re.M)
            at = anchor.end() + (end.start() if end else len(text) - anchor.end())
        else:
            at = len(text)
        block = f"\n[{section}]\n{key} = {value}\n"
        return text[:at] + block + text[at:], f"[{section}] (new section) {key} = {value}"
    ...
    if not match:
        ...
        return text[: sec.end()] + new_block + text[stop:], f"[{section}] {key}: (unset) -> {value}"
    old = match.group(2)
