...
# Tuning targets. formation_energy is the mandated encoder-tuning task; the companions carry
# real headroom (single-task ceilings 0.57 / 0.69 vs formation_energy's 0.995) and exist purely
# so the ranking is resolvable rather than noise-limited.
ENC_TASK = "formation_energy"
ENC_COMPANIONS = ["volume", "final_energy"]
...
LATENTS = [64, 128, 256]
ENC_HIDDEN = [[256], [512, 256], [1024, 512, 256]]
ENC_LRS = [1e-3, 5e-3]
...
def stage_a2(winners: list[str]) -> list[tuple[str, str]]:
    ...
    for w in winners:
        ...
        for bs in (512, 1024):
            ...
            ov = task_override(ENC_TASK) + " " + overrides(**enc, data__batch_size=bs)
            rows.append((runid, ov))
    return rows
...
def stage_a3(winner: str, batch_size: int) -> list[tuple[str, str]]:
    ...
    for n_grids in (4, 16):
        ...
        ov = task_override(ENC_TASK) + " " + overrides(
            **enc, data__batch_size=batch_size, descriptor__n_grids=n_grids
        )
        rows.append((runid, ov))
    return rows


def stage_a4(winner: str, batch_size: int) -> list[tuple[str, str]]:
    """Seed repeats for the winner AND the untuned baseline — the campaign's noise band."""
    rows = []
    arms = {"win": (parse_a1(winner), batch_size), "base": ({}, BASE["data.batch_size"])}
    for arm, (enc, bs) in arms.items():
        for seed in (2026, 2027):
            runid = f"a4_{arm}_s{seed}"
            ov = task_override(ENC_TASK) + " " + overrides(**enc, data__batch_size=bs)
...
def stage_a5(winners: list[str], batch_size: int) -> list[tuple[str, str]]:
    ...
    for w in winners:
        enc = parse_a1(w)
        for task in ENC_COMPANIONS:
            runid = f"a5_{w[3:]}_{task}"
            ov = task_override(task) + " " + overrides(**enc, data__batch_size=batch_size)
            rows.append((runid, ov))
    for task in ENC_COMPANIONS:  # untuned control on the same tasks
        rows.append((f"a5_base_{task}", task_override(task)))
    return rows


# --- stage B: task heads (encoder frozen at the stage-A winner) ------------------------------

HEAD_HIDDEN = [[64], [128, 64], [256, 128, 64]]
HEAD_LRS = [1e-3, 5e-3, 1e-2]
KR_N_KERNEL = [15, 32, 64]
KR_X_HIDDEN = [[128, 64], [256, 128, 64]]
KR_LRS = [5e-4, 2e-3]
KR_T_HIDDEN = [[16, 8], [32, 16], [64, 32, 16]]
KR_WD = [5e-5, 5e-4]
CLF_HIDDEN = [[64], [128, 64], [256, 128]]
CLF_LRS = [1e-3, 5e-3, 1e-2]
